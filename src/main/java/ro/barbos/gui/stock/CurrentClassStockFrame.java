...
import ro.barbos.gui.tablemodel.ClassStockModel;

public class CurrentClassStockFrame extends GeneralFrame implements
		ActionListener {

	
	private static final long serialVersionUID = 1L;
	ClassStockModel stockModel;
	JTable stockTable;

	private NumberFormat numberFormatter = NumberFormat.getInstance(ConfigLocalManager.locale);

	public CurrentClassStockFrame() {
		super();
		...
		stockModel = new ClassStockModel();
	...
	private void extractStock() {
		List<ClassStockDTO> lumberStockClassDTO = StockDAO
				.getCurrentStockClassInfo();
		if (lumberStockClassDTO != null) {
			stockModel.setLumberClassInfo(lumberStockClassDTO);
		}
	...
	public String getFrameCode() {
		return GUIUtil.CLASS_STOCKS_KEY;
	}
