...
import ro.barbos.gater.cutprocessor.diagram.GaterRotate;
...
public class RotateOneCutStrategy extends AbstractCutStrategy {
	...
	public CutDiagram cutLumberLog(LumberLog lumberLog, List<Product> products, List<Integer> targetPieces) {
		...
		double radius = lumberContext.getRadius();
		double currentYPosition = -radius;
		currentYPosition += CutterSettings.GATER_THICK;
				
		//cut the top segment
		while(cutInTopSegment(currentYPosition, -lumberContext.getHalfSquare() - CutterSettings.MULTIBLADE_TOLERANCE)) {
			int gaterCut = computeGaterCut(radius, currentYPosition, null);
		...
		double topLimit = 0 - currentYPosition;
		currentYPosition = -radius;
		diagram.gaterCutFlow.add(new GaterRotate());
		//rotate right
		double rightLimit = topLimit;
		if(segmentSteps.cuts.size()>0) {
		...
		while(currentYPosition < radius) {
			...
			int gaterCut = computeGaterCut(radius, currentYPosition, rightLimit);
			CutContext cutContext = lumberContext.createCutContext(gaterCut, currentYPosition);
			if(cutInTopSegment(currentYPosition, -lumberContext.getHalfSquare())) {
				...
				if(productToCut != null && productToCut.getWidth() < gaterCut) {
					...
					GaterSlide cutSlide = createGaterSlide(productToCut);
					cutSlide.phase = CutPhase.THIRD;
					cutSlide.y = currentYPosition;
					cutSlide.rightLimit = rightLimit;
					diagram.gaterCutFlow.add(cutSlide);
					int pieces = getMultibladePieces(gaterCut, productToCut.getWidth());
					cuttingState.addCutPieces(productToCut, pieces, lumberLog.getPlate());
					currentYPosition += productToCut.getThick();
					...
					cutSlide.color = colors.get(productToCut.getName());
					lastTopCut = lumberLog.getSmallRadius()/2 + currentYPosition;
					segmentSteps.addCut(productToCut.getThick().doubleValue());
				}
			}
				...
				cutContext.rightEdge = rightLimit;
				...
				if(productToCut != null) {
					...
					cutSlide.rightLimit = rightLimit;
					diagram.gaterCutFlow.add(cutSlide);
					int nextGuterCut = computeGaterCut(radius, currentYPosition + productToCut.getWidth(), rightLimit);
					int smallestCut = Math.min(gaterCut, nextGuterCut);
					int pieces = getMultibladePieces(smallestCut, productToCut.getThick());
					cuttingState.addCutPieces(productToCut, pieces, lumberLog.getPlate());
					MultibladeCutSlide cut = createMultiBladeSlide(gaterCut, currentYPosition, pieces, productToCut);
					cut.phase = CutPhase.THIRD;
					double multiBladeWidth = pieces * (productToCut.getThick() + CutterSettings.MULTIBLADE);
					cut.x = rightLimit - multiBladeWidth + CutterSettings.MULTIBLADE;
					double yLimit = Math.sqrt(radius * radius - rightLimit * rightLimit);
					if(cut.y + productToCut.getWidth() > yLimit) {
						cut.x -= (smallestCut - multiBladeWidth)/2;
						cut.x = -smallestCut/2;
					}
				...
				productToCut = cuttingState.getBestMatchSmallestProduct(cutContext);
				if(productToCut != null) {
					int nextGuterCut = computeGaterCut(radius, currentYPosition + productToCut.getThick(), rightLimit);
					int smallestCut = Math.min(gaterCut, nextGuterCut);
					if(smallestCut > productToCut.getWidth()) {
						...
						double tempWidth = cutSlide.pieces * productToCut.getWidth();
						tempWidth += (pieces-1) * CutterSettings.MULTIBLADE;
						cutSlide.x = rightLimit - tempWidth;
						double yLimit = Math.sqrt(radius * radius - rightLimit * rightLimit);
						if(cutSlide.y + productToCut.getThick() > yLimit) {
							cutSlide.x = -tempWidth/2;
						}
						cutSlide.color = colors.get(productToCut.getName());
